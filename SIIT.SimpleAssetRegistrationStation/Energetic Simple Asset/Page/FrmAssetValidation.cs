...
{
    public partial class FrmAssetValidation : Form
    {
        string logText;
        ...
        public FrmAssetValidation(FrmMain e)
        {
            InitializeComponent();
            ...
            splitContainer1.Click += splitContainer1_Panel2_Click;
            btnClear.Click += btnClear_Click;
            DgvTempAsset.Click += DgvTempAsset_Click;
            DgvTempAsset.SelectionChanged += DgvTempAsset_SelectionChanged;
            BtnCheckFromDb.Click += BtnCheckFromDb_Click;
            ...
            using (AssetWebApi.AssetServiceClient cl = new AssetWebApi.AssetServiceClient())
            {
                ...
                if (res.Flag)
                {
                    ...
                    {
                        cboRoom.Items.Add(row["ROOM_CODE"].ToString());
                    }
                }
            }
        }
                    ...
                    try
                    {
                        using (AssetWebApi.AssetServiceClient cl = new AssetWebApi.AssetServiceClient())
                        {
                            ...
                            if (res.Flag)
                            {
                                ...
                                if(RoomCode==cboRoom.Text)
                                {
                                    row.DefaultCellStyle.BackColor = Color.YellowGreen;
                                    foreach(DataGridViewRow roomRow in DgvAssetInRoom.Rows)
                                    {
                                        if(row.Cells["Asset TID"].Value.ToString().Equals(roomRow.Cells["ASSET_TID"].Value.ToString()))
                                        {
                                            roomRow.DefaultCellStyle.BackColor = Color.YellowGreen;
                                        }
                                    }
                                }
                            }
        ...
        void EnableGridButtons()
        {
            Action action = () =>
            {
                ...
                if (0 < DgvTempAsset.SelectedRows.Count) btnEditRowAndSave.Enabled = true;
                else btnEditRowAndSave.Enabled = false;
            };
            if (btnClear.InvokeRequired) btnClear.BeginInvoke(new DelegateVoid(() => { action(); }));
            else action();
        }
        ...
        public void SearchAssetByRoom()
        {
            try
            {
                using (AssetWebApi.AssetServiceClient cl = new AssetWebApi.AssetServiceClient())
                {
                    AssetWebApi.ResultModelType res = cl.SearchByRoomCode(cboRoom.Text);
                    if (res.Flag)
                    {
                        DgvAssetInRoom.DataSource = res.DataSetResult.Tables[0];
                        DgvAssetInRoom.Columns["ID"].Visible = false;
                        DgvAssetInRoom.Columns["ROOM_CODE"].Visible = false;
                        DgvAssetInRoom.Columns["ROOM_TID"].Visible = false;
                        DgvAssetInRoom.Columns["ROOM_EPC"].Visible = false;
                        DgvAssetInRoom.Columns["ROOM_DESCRIPTION"].Visible = false;
                        lblAssetCount.Text = string.Format("Found {0} asset(s).",res.DataSetResult.Tables[0].Rows.Count);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DgvTempAsset_SelectionChanged_1(object sender, EventArgs e)
        {
            EnableGridButtons();
        }
        ...
        public void ConnectReaders()
        {
            try
            {
                btnScanTags.Enabled = btnStop.Enabled = btnClear.Enabled = false;
