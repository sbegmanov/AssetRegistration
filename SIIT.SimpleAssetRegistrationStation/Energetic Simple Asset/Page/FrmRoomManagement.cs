...
namespace Energetic_Simple_Asset.Page
{
    public partial class FrmRoomManagement : Form
    {
        ...
        public FrmRoomManagement(FrmMain e)
        {
            InitializeComponent();
            ...
            DgvTempAsset.DoubleClick += DgvTempAsset_DoubleClick;
            btnEditRowAndSave.Click += btnEditRowAndSave_Click;
        ...
        void CheckFromDB()
        {
            try
            {
                ...
                {
                    // Check from web api
                    string tid = row.Cells["Room TID"].Value.ToString();
                    string RoomCode = "", RoomDescription = "";
                    try
                    {
                        using (AssetWebApi.AssetServiceClient cl = new AssetWebApi.AssetServiceClient())
                        {
                            AssetWebApi.ResultModelType res = cl.SearchByRoomTID(tid);
                            if (res.Flag)
                            {
                                RoomCode = res.ROOM_CODE;
                                RoomDescription = res.ROOM_DESCRIPTION;

                                row.Cells["Room Code"].Value = RoomCode;
                                row.Cells["Room Description"].Value = RoomDescription;
                            }
                        }
                    }
                    catch (Exception ex)
        ...
        private void UpdateListbox(ImpinjReader sender, TagOpReport report)
        {
            ...
            foreach (TagOpResult result in report)
            {
                ...
                try
                {
                    ...
                    {
                        tempAsset.Columns.AddRange(new DataColumn[6]
                        {
                            new DataColumn("No."),
                            new DataColumn("Room Code"),
                            new DataColumn("Room TID"),
                            new DataColumn("Room EPC"),
                            new DataColumn("RSSI"),
                            new DataColumn("Room Description"),
                        });
                        tempAsset.PrimaryKey = new DataColumn[] { tempAsset.Columns["Room TID"] };
                    }
                    tempAsset.Rows.Add(tempAsset.Rows.Count + 1, RoomCode,  tidData, epcData, Rssi, RoomDescription);
        ...
        void ShowDialog_EditRowAndSave()
        {
            try
            {
                var dlg = new FrmEditRowAndSaveRoom(DgvTempAsset.SelectedRows[0]);
